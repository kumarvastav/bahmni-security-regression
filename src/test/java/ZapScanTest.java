/**
 * Created by dharmens on 12/14/16.
 */
...
public class ZapScanTest {
    ...
    private final static String ZAP_APIKEY = "87tdnb0qek3d6o9sfmrg39nqjk";
    ...
    private final static String CHROME_DRIVER_PATH = "/usr/local/bin/chromedriver";
    ...
    public void setup() {
        ...
        driver = createChromeDriver(createProxyCapabilities(proxy),CHROME_DRIVER_PATH);
    ...
    @Test
    public void scanWithoutLogin(){
        driver.get("https://gauge.mybahmni.org");
    ...
    private void logAlerts(List<Alert> alerts) {
        for (Alert alert : alerts) {
            System.out.println("Alert: "+alert.getAlert()+" at URL: "+alert.getUrl()+" Parameter: "+alert.getParam()+" CWE ID: "+alert.getCweId());
        }
    }
    /*
    ...
    private List<Alert> filterAlerts(List<Alert> alerts) {
        ...
        for (Alert alert : alerts) {
            ...
            if (alert.getRisk().equals(Alert.Risk.High) && alert.getRisk().equals(Alert.Risk.Medium)) filtered.add(alert);
        }
        return filtered;
    }
    ...
    private void scanWithZap() {
        ...
        zapScanner.scan("https://gauge.mybahmni.org");
    ...
    private void spiderWithZap() {
        zapSpider.excludeFromSpider("https://gauge.mybahmni.org/bahmni/home/index.html#/login");
        zapSpider.setThreadCount(5);
        zapSpider.setMaxDepth(5);
        zapSpider.setPostForms(false);
        zapSpider.spider("https://gauge.mybahmni.org");
